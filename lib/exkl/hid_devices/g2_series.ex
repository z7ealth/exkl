defmodule Exkl.HidDevices.G2Series do
  ...
  @active_header [104, 1, 8, 12, 1, 2]
  @off_header [104, 1, 8, 12, 1, 0]

  @spec encode(%Exkl.AK{}) :: binary()
  def encode(%Exkl.AK{} = metrics), do: encode_metrics(metrics)

  @spec startup() :: binary()
  def startup, do: blank()

  @spec blank() :: binary()
  def blank, do: encode_zeros(@active_header, 0, 0.0, 0, 0)

  @spec off() :: binary()
  def off, do: encode_zeros(@off_header, 0, 0.0, 0, 0)
  ...
  defp encode_zeros(header, unit, temp, usage, freq_mhz, power_w \\ 0) do
