...
| **Monitor popup in wrong place** | Set `EXKL_TRAY_POPUP_POSITION` in `~/.config/exkl/env` to `top-right`, `top-left`, `bottom-right`, or `bottom-left`, then restart the service. |
