  ...
  defp encode_metrics(%Exkl.AK{} = metrics) do
    temp_c =
      case metrics.mode do
        :cpu_temp_f -> fahrenheit_to_celsius(metrics.metrics_value)
        _ -> metrics.metrics_value
      end
    ...
    power_mw =
      case metrics.cpu_power_w do
        watts when is_float(watts) and watts > 0 -> round(watts * 1000) |> min(65_535)
        _ -> 0
      end
    ...
    <<power_hi::8, power_lo::8>> = <<power_mw::16-big>>
    <<temp_b0::8, temp_b1::8, temp_b2::8, temp_b3::8>> = <<temp_c::float-big>>
